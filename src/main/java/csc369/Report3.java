...
public class Report3 {
	...
	protected void map(LongWritable key, Text value,
	    ...
	    Text hostname = new Text(); // hard code this.
        hostname.set("d97082.upc-d.chello.nl");
        IntWritable byte_count = new IntWritable();
        byte_count.set(Integer.parseInt(sa[9]));
        if (hostname.toString().equals(sa[0])) {
            context.write(hostname, byte_count);
        }
    }
}
